...
// =============================================================================
// PUBLIC API — server-callable getters
// =============================================================================

/**
 * Fetch, filter, dedupe and sort all basketball news articles from every RSS
 * feed. Results are cached in-memory for 5 minutes (see lib/news/cache.ts).
 * Safe to call directly from server components and route handlers.
 */
...
/**
 * Look up a single article by its (base64url-encoded) id. Pulls from the full
 * article set returned by getAllArticles(), so it works on a cold server render
 * even when the in-memory cache is empty.
 */
