...
async function fetchBdl<T>(endpoint: string, params?: Record<string, string>): Promise<T> {
  ...
  try {
    ...
    if (!(error instanceof Error && error.message.startsWith("BallDontLie API Error:"))) {
      trackApiCall({
        provider: ApiProvider.BDL,
        endpoint,
        statusCode: 0,
...
export class BallDontLieClient {
  ...
  async getTeams(): Promise<NormalizedTeam[]> {
    ...
    if (cached) {
      trackApiCall({ provider: ApiProvider.BDL, endpoint: "/teams", responseTime: 0, cached: true });
      return cached;
    }
  ...
  async getPlayer(playerId: string): Promise<NormalizedPlayer | null> {
    ...
    if (cached) {
      trackApiCall({ provider: ApiProvider.BDL, endpoint: `/players/${playerId}`, responseTime: 0, cached: true });
      return cached;
    }
  ...
  async getTodaysGames(): Promise<NormalizedGame[]> {
    ...
    if (cached) {
      trackApiCall({ provider: ApiProvider.BDL, endpoint: "/games", responseTime: 0, cached: true });
      return cached;
    }
