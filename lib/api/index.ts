...
export class BasketballApi {
  ...
  async getTeamsByLeague(league: League): Promise<NormalizedTeam[]> {
    switch (league) {
      case "nba": {
        // Try BDL first (has conference/division data), fall back to ESPN
        const bdlTeams = await nbaApi.getTeams();
        if (bdlTeams.length > 0) return bdlTeams;
        return espnApi.getNbaTeams();
      }
      case "wnba":
