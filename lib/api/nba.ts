...
import { getBdlSeason } from "./season";

const BASE_URL = "https://api.balldontlie.io/v1";

// NBA official player IDs (for headshots and stats)
// Maps player names to their official NBA.com player IDs
const NBA_PLAYER_IDS: Record<string, string> = {
  ...
  "Amen Thompson": "1641707",
  "Ausar Thompson": "1641708",
...
function normalizePlayer(player: ApiPlayer): NormalizedPlayer {
  ...
  // Try to get the official NBA ID for this player
  const nbaId = NBA_PLAYER_IDS[fullName] || playerId;
  return {
    id: playerId,
    nbaId: nbaId,
    name: fullName,
...
export class NbaApiClient {
  ...
  async getPlayerSeasonStats(
    ...
    const currentSeason = season || getBdlSeason();
  ...
  async getTeamGames(
    ...
    const season = options?.season || getBdlSeason();
