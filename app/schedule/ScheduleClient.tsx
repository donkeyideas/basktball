...
export default function ScheduleClient({
  ...
  initialWeekData?: DayGames[];
}) {
  const [league, setLeague] = useState<League>("nba");
  const [weekStart, setWeekStart] = useState<Date>(() => getMonday(new Date()));
  const [weekData, setWeekData] = useState<DayGames[]>(initialWeekData);
  const [loading, setLoading] = useState(initialWeekData.length === 0);
  // Skip the first fetch when SSR already provided the current NBA week
  const skipFirstFetch = useRef(initialWeekData.length > 0);
