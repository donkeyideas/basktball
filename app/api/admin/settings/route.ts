...
async function saveSettings(request: Request) {
  try {
    ...
    // Sync maintenance mode to Redis so edge middleware can read it without Prisma
    if ("maintenanceMode" in settings) {
      try {
        await cache.set("settings:maintenanceMode", String(settings.maintenanceMode), 86400 * 365);
      } catch {
        // Non-critical — middleware will just use stale value
      }
    }
