...
export default function StandingsClient({
  ...
  initialData?: StandingsResponse | null;
}) {
  const [selectedLeague, setSelectedLeague] = useState<League>("nba");
  const [confView, setConfView] = useState<ConferenceView>("both");
  const [data, setData] = useState<StandingsResponse | null>(initialData);
  const [isLoading, setIsLoading] = useState(!initialData);
  const [error, setError] = useState<string | null>(null);
  // Skip the first fetch when SSR already provided NBA standings
